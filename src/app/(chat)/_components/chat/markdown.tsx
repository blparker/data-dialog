...
function SimpleCodeBlock({
    ...
    // Determine if this is inline code or block code based on className
    // If className contains a language (e.g., "language-javascript"), it's a block
    // If className is empty or doesn't contain a language, it's inline
    const isInline = !className || !className.includes('language-');

    console.log('*** ', node, inline, className, children, props, 'isInline:', isInline);

    if (isInline) {
        // Inline code (single backticks)
        return (
            <code className={cn('text-sm bg-zinc-100 dark:bg-zinc-800 py-0.5 px-1 rounded-md', className)} {...props}>
                {children}
            </code>
        );
    } else {
        ...
        return (
            <span className="not-prose block rounded-md bg-zinc-100 p-2 border border-zinc-300">
